...
module.exports = function(options, issue) {
  ...
  return function password(req, res, next) {
    ...
    try {
      ...
      if (arity == 8) {
        issue(client, username, passwd, code ,scope, req.body, req.authInfo, issued);
      } else if (arity == 7) {
        issue(client, username, passwd, code,scope, req.body, issued);
      } else if (arity == 6) {
        issue(client, username, passwd,code, scope, issued);
      } else { // arity == 4
        issue(client, username, passwd,code, issued);
      }
    } catch (ex) {
      return next(ex);
    }
